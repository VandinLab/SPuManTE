...
int main(int argc, char *argv[]){
  ...
  if (argc != 6){
	  printf("LCM_LAMP_FISHER: output_basefilename target_fwer input_class_labels_file input_transactions_file epsilon\n");
	  exit(1);
  }
  ...
  tic = measureTime();
  lamp_init(sig_th,class_labels_file,epsilon);
  toc = measureTime();
